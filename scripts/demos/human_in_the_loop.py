...
async def main():
    ...
    for _ in range(args.time_steps / 2):
        env_action_empty = EnvAction(activate_agents=list(random.sample(range(110), 10)))
        await env.step(env_action_empty)


    # Human intervention at halftime
    action_2 = SingleAction(
            agent_id=0,
            action=ActionType.CREATE_COMMENT,
            args={
                "content": input("Enter Intervention: ")
                }
            )
   
    env_action_2 = EnvAction(activate_agents=list(random.sample(range(110), 10)),
                             intervention=[action_2])

    await env.step(env_action_2)
